...
def File():
    ...
    def Edit():
        def EditFile():
            if(ff.writeFileUI(inputtxt.get(1.0, "end-1c"), inputTxtTwo.get(1.0, "end-1c"))):
    ...
    def Use():
        def getFile():
            def EndOfUse():
                ...
                print(values)
                print("CODE RUN")
                mes = ff.postProcessSolve(values, fileVal)
                print(mes)
                print("MES HERE")
...
def Manual():
    def solve():
        mes = me.enterFormula((inputtxt.get(1.0, "end-1c")))
        mb.showwarning(title="Answer", message=mes)
    print("Manual")
    m.destroy()
...
greeting.pack()
button = tk.Button(m, text='Work With File', width=25, command=File)
button.pack()


buttonTwo = tk.Button(m, text='Enter Values Manually', width=25, command=Manual)
buttonTwo.pack()
